...
db = pymysql.connect("localhost", "root", "", "ubl")
...
def customers_show():
    ...
    if session.get('ids') ==None:
        calls_list = cursor.execute("Select * from calls")
        data1 = cursor.fetchall()
        for row in data1:        
    ...
    row_count = cursor.execute('SELECT * FROM customers where (call_status="" or call_status="0" or call_status="cl" or intrest_level="cl") and(id not in('+cids[:-1]+')) order by score desc limit 1')
    print('SELECT * FROM customers where (call_status="" or call_status="0" or call_status="cl" or intrest_level="cl") and(id not in('+cids[:-1]+')) order by score desc limit 1')
...
def call_status_update():
    ...
    pid=request.form['id']
    opid=session["user_id"]
    print(len(session['callers_ids']))
    if page=='calls': 
        if len(session['callers_ids']) >0:       
            cursor = db.cursor()
            cursor.execute("update customers set call_status=%s, intrest_level='%s', comments='%s', caller_id=%s where id=%s"%(call,status,comment,opid,pid))
            db.commit()
            print(session['ids'])
            print(session['callers_ids'])
